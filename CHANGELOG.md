...
Security fix to enforce disk network access policies for managed disks.
...
* Fix Azure disk network access to `DenyAll` and public network access to `Disabled` by default for all managed disks (OS and data). Implemented via azapi provider PATCH operations in `main.tf` after VM creation. Re-applying terraform on existing deployments will update disk network access policies.

# v2.0.1 (2026-03-25)

## Release Summary

Minor fixes to the Terraform module configuration and documentation.
...
* Fix marketplace SKUs to use free cluster plan (`apexfilestorage_cluster`) instead of paid plan SKUs, per the Dell PowerScale for Microsoft Azure Deployment Guide. Updated all three regions (US, International, Canada) in both `variables.tf` and `docs/POWERSCALE_CLUSTER.md`.
* Fix `terraform.tfvars.template`: change HTML comment delimiters to HCL block comments, add missing `location` variable, and add missing quotes around `resource_group` placeholder value.
